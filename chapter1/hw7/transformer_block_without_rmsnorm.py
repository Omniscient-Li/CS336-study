...
class TransformerBlock(nn.Module):
    ...
    def __init__(self , d_model : int , n_heads : int , d_ff : int , max_seq_len : int , theta : float , device = None):
        ...
        self.theta = theta
        self.device = device
        # "无 RMSNorm" 实验版本：把两个 RMSNorm 换成标准 LayerNorm，其余结构不变
        self.rms_norm1 = nn.LayerNorm(d_model , eps = 1e-5 , device = device)
        self.rms_norm2 = nn.LayerNorm(d_model , eps = 1e-5 , device = device)
        self.swiglu = SwiGLU(d_model , d_ff)
        self.casual_multi_head_attention = CausalMultiHeadAttentionNoWeight(d_model , n_heads , max_seq_len , theta , device)

    def forward(self , in_features : torch.Tensor):
        token_positions = torch.arange(in_features.shape[1] , device = in_features.device)
        x1 = self.rms_norm1(in_features)
        x1 = self.casual_multi_head_attention(x1 , token_positions)
        x1 = x1 + in_features
