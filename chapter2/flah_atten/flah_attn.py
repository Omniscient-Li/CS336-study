...
def _attn_fwd_inner(
    ...
    if STAGE == 1:
        # From 0 to the left of the diagnoal
        lo , hi = 0 , block_index_q * BLOCK_SIZE_Q
    elif STAGE == 2:
        # Used only for the block in which there is transition between non_masked and masked keys
        lo , hi = block_index_q * BLOCK_SIZE_Q , (block_index_q + 1) * BLOCK_SIZE_Q
        lo = tl.multiple_of(lo , BLOCK_SIZE_Q)
    else: 
        # Only used for non_causal atention
        lo , hi = 0 , SEQ_LEN
    ...
    for start_kv in range(lo , hi , BLOCK_SIZE_KV):
        #Just let the complier know that start_n is a multiple of BLOCK_N , so the complier can do optimizations
...
def _attn_fwd(
    ...
    # STAGE 1 无掩码处理对角块左侧，要求 hi=block_index_q*BLOCK_SIZE_Q 是 BLOCK_SIZE_KV 的整数倍
    # （否则会多扫入"未来"的 key）；BLOCK_SIZE_KV=BLOCK_SIZE_Q=32 时恒成立
    ...
    # This allows to get the (SEQ_LEN , HEAD_DIM) in hte Q , K , V by selecting indexing it by batch and head
    qvk_offset = (
        index_batch.to(tl.int64) * stride_Q_batch
        + index_head.to(tl.int64) * stride_Q_head
    )

    Q_block_ptr = tl.make_block_ptr(  #Q[index_batch , index_head , block_indexc_q * BLOCJ_SIZE_Q : ]
        base = Q + qvk_offset ,
    ...
    V_block_ptr = tl.make_block_ptr( # V[index_batch_size , index_head , : , : , ]
        base=V + qvk_offset ,
        shape=(SEQ_LEN , HEAD_DIM) ,
    ...
    # epilogue（两种 STAGE 都必须执行：O 除以 l_i 归一化 + 存 logsumexp 供 backward 用）
    m_i += tl.math.log(
        l_i
    ...
    tl.store(m_ptrs , m_i)
    tl.store(O_block_ptr , O_block.to(O.dtype.element_ty))
...
def _attn_bwd_preprocess(
    ...
    O_block = tl.load( # O (BATCH_SIZE , NUM_HEAD , SEQ_LEN , HEAD_DIM)
        O 
        + index_batch_head * HEAD_DIM * SEQ_LEN
        + offs_q[: , None] * HEAD_DIM
        + offs_dim[None , :]
    ) # (BLOCK_SIEZEQ , HEAD_DIM)
...
class TritonAttention(torch.autograd.Function):
    ...
    def forward(ctx , Q , K , V , causal , softmax_scale):
        ...
        grid = lambda args: (
            ...
            BATCH_SIZE * NUM_HEADS , # Which head of batch elment are we going to work with
            1 , # z in CUDA launch grid
        )
