...
def flash_fwd_kernel(
    ...
    Q_i = tl.load(Q_block_ptr) # shape: (Q_TILE_SIZE, D)
    O_i_acc = tl.zeros((Q_TILE_SIZE , D) , dtype = tl.float32) #shape : (Q_TILE_SIZE, D)
    L_i_acc = tl.zeros((Q_TILE_SIZE , 1) , dtype = tl.float32)
    #shape : (Q_TILE_SIZE, 1)
    M_i_acc = tl.full((Q_TILE_SIZE , 1) , float('-inf') , dtype = tl.float32) #shape : (Q_TILE_SIZE, 1)
    # 外层循环：对 K/V 的序列长度进行分块
    for j in range(tl.cdiv(N_KEYS , K_TILE_SIZE)):
        K_j = tl.load(K_block_ptr) # shape: (K_TILE_SIZE, D)
        V_j = tl.load(V_block_ptr) # shape: (K_TILE_SIZE, D)
        #计算attention scores
        S_ij = tl.dot(Q_i , K_j.T) * scale # shape: (Q_TILE_SIZE, K_TILE_SIZE)
        ...
        if is_causal :
            ...
            """
            q_idx = i * Q_TILE_SIZE + tl.arange(0 , Q_TILE_SIZE)[ : , None] # shape: (Q_TILE_SIZE, 1)
            k_idx = j * K_TILE_SIZE + tl.arange(0 , K_TILE_SIZE)[ None , :] # shape: (1, K_TILE_SIZE)
            casual_mask = q_idx >= k_idx # shape: (Q_TILE_SIZE, K_TILE_SIZE)
            S_ij = tl.where(casual_mask , S_ij , -1e6)

        M_ij = tl.max(S_ij , axis = 1 , keep_dims = True) # shape: (Q_TILE_SIZE, 1) 当前块的最大值
        M_i_new = tl.maximum(M_i_acc , M_ij) # shape: (Q_TILE_SIZE, 1) 当前块的最大值和之前块的最大值进行比较，得到新的最大值
        P_ij = tl.exp(S_ij - M_ij) # shape: (Q_TILE_SIZE, K_TILE_SIZE) - 注意：用M_ij而不是M_i_new
        L_i_new = tl.exp(M_i_acc - M_i_new) * L_i_acc + tl.exp(M_ij - M_i_new) * tl.sum(P_ij , axis = 1 , keep_dims = True) # shape: (Q_TILE_SIZE, 1)
        #数据对齐
        P_ij_cast = P_ij.to(V_j.dtype) # shape: (Q_TILE_SIZE, K_TILE_SIZE)
        O_i_new = tl.exp(M_i_acc - M_i_new) * O_i_acc + tl.exp(M_ij - M_i_new) * tl.dot(P_ij_cast , V_j) # shape: (Q_TILE_SIZE, D)
...
def flash_bwd_kernel(
    ...
    for i in range(tl.cdiv(N_QUERIES , Q_TILE_SIZE)):
        # 加载当前Q tile的所有相关数据
        Q_i = tl.load(Q_block_ptr) # shape: (Q_TILE_SIZE, D)
        O_i = tl.load(O_block_ptr) # shape: (Q_TILE_SIZE, D)
        dO_i = tl.load(dO_block_ptr) # shape: (Q_TILE_SIZE, D)
        L_i = tl.load(L_block_ptr) # shape: (Q_TILE_SIZE, 1)

        # 加载预计算的D_i = rowsum(dO_i ○ O_i)，按照伪代码
        D_i = tl.load(D_block_ptr) #(Q_TILE_SIZE, 1)

        #计算attention scores S_ij = Q_i @ K_j.T / (d_k ** 0.5)
        S_ij = tl.dot(Q_i , K_j.T) * scale # shape: (Q_TILE_SIZE, K_TILE_SIZE)
        ...
        if is_causal:
            ...
            casual_mask = q_idx >= k_idx #(Q_TILE_SIZE , K_TILE_SIZE) 这里比较的思路就是把每次的偏移之后的行、列索引都进行比较，如果q_idx>=k_idx，则代表S_ij处在大S矩阵的上三角区域，所以需要把S_ij变成-inf
            S_ij = tl.where(casual_mask , S_ij ,-1e6)

        P_ij = tl.exp(S_ij - L_i) # shape: (Q_TILE_SIZE, K_TILE_SIZE)

        dV_j += tl.dot(P_ij.to(V_j.dtype).T , dO_i) # shape: (K_TILE_SIZE, D)
        dP_ij = tl.dot(dO_i ,V_j.T) # shape: (Q_TILE_SIZE, K_TILE_SIZE)

        dS_ij = P_ij * (dP_ij - D_i) * scale # shape: (Q_TILE_SIZE, K_TILE_SIZE)
        dS_ij = dS_ij.to(Q_i.dtype) # shape: (Q_TILE_SIZE, K_TILE_SIZE)
...
class FlashAttentionAutogradFunctionTriton(torch.autograd.Function):
    ...
    def backward(ctx , do):
        ...
        D_sum = torch.sum(o * do , dim = -1 , keepdim = True) #(batch_size , Nq , 1) 计算D = rowsum(dO ○ O)
        grid = (Tk, batch_size)
