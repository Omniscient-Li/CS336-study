...
def flash_fwd_kernel(
    ...
    #这里的program_id(0)和program_id(1)分别对应启动triton kernel的grid的第一个维度和第二个维度
    # program_id(0)对应batch维度，program_id(1)对应query维度
    i = tl.program_id(0) # batch维度
    batch_index = tl.program_id(1)

    #这里offset每个指针，对应batch index乘batch stride
    ...
    K_block_ptr = tl.make_block_ptr(
        ...
        offsets = (0 , 0) , # K , V的初始化偏移为0，他们在后续循环中更新
    ...
    for j in range(tl.cdiv(N_KEYS , K_TILE_SIZE)):
        ...
        M_ij = tl.max(S_ij , axis =- 1 , keep_dims = True) #(Q_TILE_SIZE , 1) 当前快的最大值
        M_i_new = tl.maximum(M_i_acc , M_ij) #(Q_TILE_SIZE , 1) 当前块的最大值和之前块的最大值进行比较，得到新的最大值
        P_ij = tl.exp(S_ij - M_ij) #(Q_TILE_SIZE , K_TILE_SIZE) 当前块的exp(S_ij - M_ij)
