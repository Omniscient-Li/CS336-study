...
def setup(rank , world_size , backend):
    ...
    # 初始化进程组，rank是当前进程的rank，world_size是总进程数，backend: 这是指定通信后端的参数。常见的后端有：gloo(CPU), nccl(GPU), mpi等。
...
def ddp_train(rank , world_size , backend):
    ...
    # 为每个进程分配不同的GPU设备
    # 单卡实例：所有 rank 映射到同一张卡（rank % 卡数），配合 gloo 后端
    ...
    if torch.cuda.is_available():
        ...
        print(f"CUDA not avalible , using CPU for rank {rank}")
...
def main():
    world_size = 2
    backend = 'gloo'  # 单卡多进程必须 gloo（NCCL 要求每 rank 独占 GPU）；多卡时换回 nccl
