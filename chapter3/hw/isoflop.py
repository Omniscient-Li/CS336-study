...
# 基于脚本位置找数据：chapter3/hw/ 的上级目录是 chapter3/，数据在 chapter3/data/
# 这样无论从哪个目录运行都不会找不到文件（也不怕反斜杠转义）
isojson_path = Path(__file__).parent.parent / 'data' / 'isoflops_curves.json'
...
plt.xlabel('log10 C (FLOPs)')
plt.ylabel('log10 N_opt (parameters)')
plt.legend()
